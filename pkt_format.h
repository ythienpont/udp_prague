...
#define PKT_BUFFER_SIZE                                                        \
  65536 // [RFC8888] calculated using arithmetic modulo 65536
...
struct DataMessage {
  ...
  TimeUs echoed_timestamp; // echoed_timestamp can be used to calculate the RTT
  Count seq_nr; // packet sequence number, should start with 1 and increase
                // monotonic with packets sent
...
struct FrameMessage {
  ...
  Count seq_nr;     // packet sequence number, should start with 1 and increase
                    // monotonic with packets sent
  Count frame_nr;   // frame sequence number, also start with 1 and increase
                    // monitonically
  Count frame_sent; // frame sent before this packets
  Count frame_size; // frame size in bytes
...
struct AckMessage {
  ...
  void get_stat(pktsend_tp *pkts_stat, Count &m_packets_lost) {
    ack_seq = htonl(ack_seq);
    timestamp = htonl(timestamp);
    echoed_timestamp = htonl(echoed_timestamp);
    packets_received = htonl(packets_received);
  ...
  void get_frame_stat(pktsend_tp *pkts_stat, Count &m_packets_lost,
                      bool is_sending, Count frm_sending, Count &recv_frame,
                      Count &lost_frame, Count *frm_idx, Count *frm_pktsent,
                      Count *frm_pktlost) {
    ack_seq = htonl(ack_seq);
    timestamp = htonl(timestamp);
    echoed_timestamp = htonl(echoed_timestamp);
    ...
    Count frm_index = frm_idx[ack_seq % PKT_BUFFER_SIZE];
...
struct Rfc8888Ack {
  uint8_t type;
  Count begin_seq; // Use 32-bit sequence number
  uint16_t num_reports;
  uint16_t report[REPORT_SIZE];

  uint16_t get_size(uint16_t rptsize) {
    return sizeof(uint16_t) * rptsize + sizeof(type) + sizeof(begin_seq) +
           sizeof(num_reports);
  }
  uint16_t get_stat(TimeUs now, TimeUs *sendtime, TimeUs *pkts_rtt, Count &rcvd,
                    Count &lost, Count &mark, bool &error,
                    pktsend_tp *pkts_stat, Count &last_ack) {
    uint16_t num_rtt = 0;
    ...
    for (uint16_t i = 0; i < num_reports; i++, last_ack++) {
      ...
      if ((report[i] & 0x8000) >> 15) {
        if (pkts_stat[idx] == snd_sent || pkts_stat[idx] == snd_lost) {
          ...
          pkts_rtt[num_rtt++] =
              now - ((report[i] & 0x1FFF) << 10) - sendtime[idx];
  ...
  uint16_t get_frame_stat(TimeUs now, TimeUs *sendtime, TimeUs *pkts_rtt,
                          Count &rcvd, Count &lost, Count &mark, bool &error,
                          pktsend_tp *pkts_stat, Count &last_ack,
                          bool is_sending, Count frm_sending, Count &recv_frame,
                          Count &lost_frame, Count *frm_idx, Count *frm_pktsent,
                          Count *frm_pktlost) {
  ...
  uint16_t set_stat(Count &seq, Count maxseq, TimeUs now, TimeUs *recvtime,
                    Ecn *recvecn, pktrecv_tp *recvseq, SizeB maxpkt) {
    uint16_t rptsize = sizeof(type) + sizeof(begin_seq) + sizeof(num_reports);
    uint16_t reports =
        maxseq - seq > (Count)((maxpkt - rptsize) / sizeof(uint16_t))
            ? (Count)((maxpkt - rptsize) / sizeof(uint16_t))
            : maxseq - seq;
    begin_seq = seq;
    for (uint16_t i = 0; i < reports; i++, seq++) {
      uint16_t idx = (begin_seq + i) % PKT_BUFFER_SIZE;
      if (recvseq[idx] == rcv_recv ||
          (recvseq[idx] == rcv_ackd && recvtime[idx] + RCV_TIMEOUT - now > 0)) {
        report[i] = htons((0x1 << 15) + ((recvecn[idx] & ecn_ce) << 13) +
                          (((now - recvtime[idx] + (1 << 9)) >> 10) & 0x1FFF));
