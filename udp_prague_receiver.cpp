...
int main(int argc, char **argv) {
  ...
  struct DataMessage &data_msg =
      (struct DataMessage &)(receivebuffer); // overlaying the receive buffer
  struct AckMessage ack_msg;                 // the send buffer

  // create a PragueCC object. No parameters needed if only ACKs are sent
  PragueCC pragueCC;
  TimeUs now = pragueCC.Now(); // for reporting only
  Ecn new_ecn;

  // RFC8888 buffer
  struct Rfc8888Ack rfc8888_ackmsg;
  Count start_seq = 0, end_seq = 0;
  TimeUs rfc8888_acktime = now + app.rfc8888_ackperiod;
  TimeUs recvtime[PKT_BUFFER_SIZE] = {0};
  Ecn recvecn[PKT_BUFFER_SIZE] = {ecn_not_ect};
  ...
  while (true) {
    ...
    Ecn rcv_ecn = ecn_not_ect;
    SizeB bytes_received = 0;
    TimeUs waitTime =
        (app.rfc8888_ack && start_seq != end_seq)
            ? ((rfc8888_acktime - now > 0) ? (rfc8888_acktime - now) : 1)
            : 0;
    ...
    if (bytes_received != 0) {
      ...
      if (app.rfc8888_ack) {
        ...
        if (!(recvseq[seq_idx] == rcv_recv)) {
          ...
          recvecn[seq_idx] = Ecn(rcv_ecn & ecn_ce);
    ...
    if (!app.rfc8888_ack) {
      ...
      pragueCC.GetTimeInfo(ack_msg.timestamp, ack_msg.echoed_timestamp,
                           new_ecn);
      pragueCC.GetACKInfo(ack_msg.packets_received, ack_msg.packets_CE,
                          ack_msg.packets_lost, ack_msg.error_L4S);
